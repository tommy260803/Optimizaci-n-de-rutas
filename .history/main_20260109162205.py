...
if __name__ == "__main__":
    try:
        ...
        # Agregar archivos CSS y JS personalizados directamente en el HTML
        try:
            css_path = os.path.join(os.path.dirname(__file__), 'app', 'assets', 'custom_styles.css')
            js_path = os.path.join(os.path.dirname(__file__), 'app', 'assets', 'custom_scripts.js')

            if os.path.exists(css_path):
                with open(css_path, 'r', encoding='utf-8') as f:
                    css_content = f.read()
                app.index_string = app.index_string.replace('</head>', f'<style>{css_content}</style></head>')
                print("✅ CSS personalizado cargado")
            else:
                print("⚠️  Archivo CSS personalizado no encontrado")

            if os.path.exists(js_path):
                with open(js_path, 'r', encoding='utf-8') as f:
                    js_content = f.read()
                app.index_string = app.index_string.replace('</body>', f'<script>{js_content}</script></body>')
                print("✅ JS personalizado cargado")
            else:
                print("⚠️  Archivo JS personalizado no encontrado")

        except Exception as e:
            print(f"⚠️  Error al cargar archivos personalizados: {str(e)}")
