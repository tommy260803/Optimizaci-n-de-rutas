...
class AlgoritmoGenetico:
    ...
    def __init__(
        ...
        self.mejores_individuos = []  # Lista de mejores individuos únicos encontrados
        self.historial = []
    ...
    def evolucionar_generacion(self) -> None:
        ...
        if mejor_fitness_actual < self.mejor_fitness:
            ...
            # Agregar a la lista de mejores individuos únicos (si no está ya)
            individuo_existe = False
            for ind, _ in self.mejores_individuos:
                if ind == mejor_actual:
                    individuo_existe = True
                    break

            if not individuo_existe:
                self.mejores_individuos.append((mejor_actual.copy(), mejor_fitness_actual))
                # Mantener solo los 10 mejores
                self.mejores_individuos.sort(key=lambda x: x[1])  # Ordenar por fitness
                self.mejores_individuos = self.mejores_individuos[:10]  # Mantener top 10
        ...
        # Calcular diversidad como desviación estándar de los fitness
        diversidad = np.std(fitness_values) if len(fitness_values) > 1 else 0.0
    ...
    def obtener_estadisticas(self) -> Dict[str, Any]:
        ...
        # Calcular diversidad como desviación estándar de los fitness
        if self.poblacion:
            fitness_poblacion = [self.evaluar_individuo(ind) for ind in self.poblacion]
            diversidad = np.std(fitness_poblacion) if len(fitness_poblacion) > 1 else 0.0
        else:
            diversidad = 0.0
        ...
        return {
            ...
            'mejores_individuos': [ind for ind, _ in self.mejores_individuos],  # Lista de individuos
