...
class GeneradorDatos:
    ...
    def __init__(self, centro_lat=-8.1117, centro_lon=-79.0288, radio_km=8):
        """
        ...
        self.radio_km = radio_km  # Reducido de 15km a 8km para mantener puntos dentro de la ciudad

        # Límites realistas para Trujillo (evitando el mar)
        self.limites_lat = (-8.17, -8.06)  # Sur a Norte
        self.limites_lon = (-79.07, -79.00)  # Oeste a Este (costa)

        self.rng = np.random.default_rng()
        
    def generar_puntos_aleatorios(self, n_puntos=20, seed=None):
        """
        Genera puntos aleatorios dentro de los límites realistas de Trujillo.
        ...
        puntos = [{
            'id': 0,
            'nombre': 'Depósito Central Trujillo',
            'lat': self.centro[0],
            'lon': self.centro[1],
            'demanda': 0,
        ...
        # Áreas realistas de Trujillo (distritos principales)
        areas_trujillo = [
            # Centro Histórico
            {'lat_range': (-8.115, -8.105), 'lon_range': (-79.035, -79.025), 'nombre': 'Centro'},
            # Trujillo Viejo
            {'lat_range': (-8.125, -8.115), 'lon_range': (-79.045, -79.035), 'nombre': 'Trujillo Viejo'},
            # Mansiche
            {'lat_range': (-8.095, -8.085), 'lon_range': (-79.025, -79.015), 'nombre': 'Mansiche'},
            # La Esperanza
            {'lat_range': (-8.085, -8.075), 'lon_range': (-79.015, -79.005), 'nombre': 'La Esperanza'},
            # El Porvenir
            {'lat_range': (-8.075, -8.065), 'lon_range': (-79.005, -78.995), 'nombre': 'El Porvenir'},
            # Florencia de Mora
            {'lat_range': (-8.065, -8.055), 'lon_range': (-78.995, -78.985), 'nombre': 'Florencia'},
            # Victor Larco Herrera
            {'lat_range': (-8.135, -8.125), 'lon_range': (-79.055, -79.045), 'nombre': 'Larco'},
            # Huanchaco (área urbana)
            {'lat_range': (-8.075, -8.065), 'lon_range': (-79.055, -79.045), 'nombre': 'Huanchaco'},
        ]

        for i in range(1, n_puntos):
            # Seleccionar un área aleatoria de Trujillo
            area = self.rng.choice(areas_trujillo)

            # Generar coordenadas dentro del área seleccionada
            lat = self.rng.uniform(area['lat_range'][0], area['lat_range'][1])
            lon = self.rng.uniform(area['lon_range'][0], area['lon_range'][1])

            # Asegurar que esté dentro de límites generales de Trujillo
            lat = np.clip(lat, self.limites_lat[0], self.limites_lat[1])
            lon = np.clip(lon, self.limites_lon[0], self.limites_lon[1])

            puntos.append({
                'id': i,
                'nombre': f'Punto {i} - {area["nombre"]}',
                'lat': lat,
                'lon': lon,
