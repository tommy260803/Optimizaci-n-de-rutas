...
def crear_visualizacion_ruta_completa(df_puntos: 'pd.DataFrame', ruta_optimizada: List[int]) -> go.Figure:
    ...
    # Agregar números de secuencia en cada punto con máxima visibilidad
    for i, punto_id in enumerate(ruta_completa):
        ...
        if punto_id == 0:
            ...
            color_marcador = '#dc2626'  # Rojo
            color_texto = 'white'  # Texto blanco para alto contraste
            simbolo = f'{i}'
            tamano = 20
            tamano_fuente = 14

        # Crear marcador con borde blanco para mejor visibilidad
        fig.add_trace(
            go.Scattermapbox(
                lat=[float(punto['lat'])],
                lon=[float(punto['lon'])],
        ...
        # Agregar texto separado para mejor control
        fig.add_trace(
            go.Scattermapbox(
                lat=[float(punto['lat'])],
                lon=[float(punto['lon'])],
