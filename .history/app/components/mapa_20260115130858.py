...
def crear_visualizacion_ruta_completa(df_puntos: 'pd.DataFrame', ruta_optimizada: List[int]) -> go.Figure:
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                marker=dict(
                    ...
                    color='#2563eb',
                    symbol='arrow-up',
                    angle=90  # Ajustar ángulo según dirección
                ),
                name=f'→ Paso {i+1} a {i+2}',
                hovertext=f'Paso {i+1} → {i+2}',
                hoverinfo='text',
                showlegend=False
            )
        )
    ...
    fig.update_layout(
        ...
        title=dict(
            text='<b>🚛 Ruta Óptima Completa - Secuencia de Visita</b>',
            x=0.5,
            y=0.95,
            xanchor='center',
            yanchor='top',
