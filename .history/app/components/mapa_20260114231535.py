...
def agregar_puntos_entrega(fig: go.Figure, df_puntos: 'pd.DataFrame') -> go.Figure:
    ...
    for _, punto in puntos_entrega.iterrows():
        ...
        lons.append(punto['lon'])
        tamanos.append(tamano)
        colores.append(config_distrito['color'])
        textos.append(f"{config_distrito['icono']} {punto['id']}")
        tooltips.append(tooltip_info)
        nombres_legend.append(f"{config_distrito['icono']} {punto['nombre']}")

    # Agregar una sola traza con todos los puntos diferenciados
    if lats:  # Solo si hay puntos
        fig.add_trace(
            go.Scattermapbox(
                lat=lats,
                lon=lons,
