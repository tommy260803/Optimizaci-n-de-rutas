...
def actualizar_grafico_convergencia(historial: List[Dict[str, Any]]) -> go.Figure:
    ...
    # Extraer y ordenar datos del historial por generación
    datos_ordenados = sorted(historial, key=lambda x: x.get('generacion', 0))
    generaciones = [h.get('generacion', 0) for h in datos_ordenados]
    mejores_fitness = [h.get('mejor_fitness', 0) for h in datos_ordenados]
    fitness_promedio = [h.get('fitness_promedio', 0) for h in datos_ordenados]

    fig = go.Figure()

    # Agregar línea de mejor fitness (línea continua)
    fig.add_trace(
        go.Scatter(
            x=generaciones,
            y=mejores_fitness,
            mode='lines+markers',
            name='Mejor Fitness',
            line=dict(color='green', width=2, shape='linear'),  # shape='linear' asegura línea continua
            marker=dict(size=4, symbol='circle')
        )
    )

    # Agregar línea de fitness promedio (línea continua)
    fig.add_trace(
        go.Scatter(
            x=generaciones,
            y=fitness_promedio,
            ...
            line=dict(color='orange', width=2, dash='dash', shape='linear'),  # shape='linear' asegura línea continua
    ...
    fig.update_layout(
        ...
        xaxis_title='Generación',
        yaxis_title='Valor de Fitness (menor es mejor)',
        template='plotly_white',
        height=400,
        margin={'l': 50, 'r': 20, 't': 50, 'b': 50},
        showlegend=True,
        ...
        xaxis=dict(
            tickmode='linear',
            tick0=0,
            dtick=max(1, len(generaciones) // 10)  # Ajustar ticks dinámicamente
        )
    )
