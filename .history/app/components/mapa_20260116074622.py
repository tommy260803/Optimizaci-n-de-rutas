...
def crear_visualizacion_ruta_completa(df_puntos: 'pd.DataFrame', ruta_optimizada: List[int]) -> go.Figure:
    ...
    # Agregar números de secuencia en cada punto con centrado mejorado
    for i, punto_id in enumerate(ruta_completa):
        ...
        if punto_id == 0:
            ...
            simbolo = 'DEP'
            tamano = 20
            tamano_fuente = min(10, tamano // 2)  # Fuente más pequeña para "DEP"
        else:
            color_numero = '#dc2626'
            simbolo = f'{i}'
            tamano = 16
            tamano_fuente = min(14, tamano // 1.2)  # Fuente proporcional al marcador
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                lon=[float(punto['lon'])],
                mode='markers+text',
                marker=dict(size=tamano, color=color_numero, symbol='circle'),
                text=[simbolo],
                ...
                hovertext=f'<b>{simbolo} {punto["nombre"]}</b><br>ID: {punto_id}<br>Orden: {i+1}',
                hoverinfo='text',
                showlegend=False
            )
        )

    # Agregar flechas indicando la dirección con ángulos calculados
    for i in range(len(coordenadas_ruta) - 1):
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(
                    size=14,  # Tamaño aumentado para mejor visibilidad
                    color='#2563eb',
                    symbol='arrow',
                    angle=angulo  # Ángulo calculado correctamente
                ),
                name=f'Paso {i+1} → {i+2}',
                hovertext=f'<b>Paso {i+1} → {i+2}</b><br>De punto {punto_origen} a {punto_destino}<br>Dirección: {angulo:.1f}°',
                hoverinfo='text',
                showlegend=False
            )
        )
