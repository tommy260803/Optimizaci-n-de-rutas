    ...
    def callback_actualizar_barra_progreso(historial, estado_ag):
        ...
        try:
            if estado_ag:
                ...
                # Última mejora (última generación donde se encontró mejor fitness)
                if historial and 'mejores_fitness' in historial and len(historial['mejores_fitness']) > 0:
                    # Encontrar la generación de la última mejora
                    mejores_fitness = historial['mejores_fitness']
                    ultima_generacion_mejora = 0

                    for i in range(1, len(mejores_fitness)):
                        if mejores_fitness[i] < mejores_fitness[i-1]:  # Menor fitness es mejor
                            ultima_generacion_mejora = historial.get('generaciones', [j for j in range(len(mejores_fitness))])[i]
