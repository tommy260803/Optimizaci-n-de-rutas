        ...
        try:
            ...
            try:
                if hasattr(ag_instancia, 'obtener_estadisticas'):
                    ...
                    # Agregar al historial - siempre agregar aunque algunos valores sean None
                    if stats:
                        # Usar valores del historial del algoritmo si están disponibles
                        if hasattr(ag_instancia, 'historial') and ag_instancia.historial:
                            ultimo_historial = ag_instancia.historial[-1]
                            historial['mejores_fitness'].append(ultimo_historial.get('mejor_fitness', stats.get('mejor_fitness', 0)))
                            historial['fitness_promedio'].append(ultimo_historial.get('promedio_fitness', stats.get('fitness_promedio', 0)))
                            historial['peores_fitness'].append(ultimo_historial.get('peor_fitness', stats.get('peor_fitness', 0)))
                            historial['generaciones'].append(ag_instancia.generacion_actual)
                        else:
                            # Fallback a las estadísticas directas
                        ...
                        if stats.get('mejor_individuo') or (hasattr(ag_instancia, 'mejor_individuo') and ag_instancia.mejor_individuo):
                            mejor_individuo = stats.get('mejor_individuo') or ag_instancia.mejor_individuo
                            if mejor_individuo:
                                historial['mejores_individuos'].append(mejor_individuo)

                        print(f"DEBUG: Historial actualizado - Gen: {ag_instancia.generacion_actual}, Fitness: {stats.get('mejor_fitness', 0):.4f}, Total datos: {len(historial['mejores_fitness'])}")

            except Exception as stats_error:
                print(f"Error al obtener estadísticas: {str(stats_error)}")
                # Intentar obtener datos básicos del algoritmo
                try:
                    if hasattr(ag_instancia, 'historial') and ag_instancia.historial:
                        ultimo_historial = ag_instancia.historial[-1]
                        historial['mejores_fitness'].append(ultimo_historial.get('mejor_fitness', 0))
                        historial['fitness_promedio'].append(ultimo_historial.get('promedio_fitness', 0))
                        historial['peores_fitness'].append(ultimo_historial.get('peor_fitness', 0))
                        historial['generaciones'].append(ag_instancia.generacion_actual)
                        if 'mejores_individuos' not in historial:
                            historial['mejores_individuos'] = []
                        if hasattr(ag_instancia, 'mejor_individuo') and ag_instancia.mejor_individuo:
                            historial['mejores_individuos'].append(ag_instancia.mejor_individuo)
                        print(f"DEBUG: Historial actualizado desde atributo - Gen: {ag_instancia.generacion_actual}")
                except Exception as fallback_error:
                    print(f"Error en fallback de estadísticas: {str(fallback_error)}")
