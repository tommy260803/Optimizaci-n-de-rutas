...
def registrar_callbacks_disenador(app):
    ...
    try:
        ...
        # 3. Callback genérico para botones dinámicos
        @app.callback(
            [Output("store-dataset-manual", "data", allow_duplicate=True),
        ...
        def manejar_botones_dinamicos(btns_eliminar, btns_editar, dataset_actual):
            """
            Maneja tanto eliminación como edición de puntos con IDs dinámicos.
            """
            from dash import ctx
            ...
            if 'btn-eliminar-punto' in button_id:
                # Es un botón de eliminar
                id_punto = int(button_id.split('"index":')[1].split('}')[0])
                dataset_filtrado = [p for p in dataset_actual if p.get('id') != id_punto]
                return [dataset_filtrado] + [no_update] * 8

            elif 'btn-editar-punto' in button_id:
                # Es un botón de editar
                id_punto = int(button_id.split('"index":')[1].split('}')[0])
