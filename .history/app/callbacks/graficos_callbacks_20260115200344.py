...
def registrar_graficos_callbacks(app):
    ...
    def callback_actualizar_convergencia(historial: Optional[Dict]) -> Dict:
        ...
        """
        print(f"DEBUG Gráfico: Historial recibido = {historial}")

        if not historial or 'mejores_fitness' not in historial or not historial['mejores_fitness']:
            print("DEBUG Gráfico: No hay datos suficientes, mostrando mensaje")
            # Retornar gráfico con mensaje informativo si no hay datos
            from app.components.graficos import crear_grafico_sin_datos
            return crear_grafico_sin_datos().to_dict()

        try:
            print(f"DEBUG Gráfico: Procesando {len(historial['mejores_fitness'])} puntos de fitness")

            # Formatear el historial para el gráfico
            datos_grafico = []
            for i in range(len(historial['mejores_fitness'])):
                datos_grafico.append({
                    'generacion': historial.get('generaciones', [])[i] if 'generaciones' in historial and i < len(historial['generaciones']) else i,
                    'mejor_fitness': historial['mejores_fitness'][i],
                    'fitness_promedio': historial.get('fitness_promedio', [0])[i] if 'fitness_promedio' in historial and i < len(historial['fitness_promedio']) else 0,
                    'peor_fitness': historial.get('peores_fitness', [0])[i] if 'peores_fitness' in historial and i < len(historial['peores_fitness']) else 0,
                    'diversidad': historial.get('diversidad', [0])[i] if 'diversidad' in historial and i < len(historial['diversidad']) else 0
                })

            print(f"DEBUG Gráfico: Datos formateados = {datos_grafico[:3]}...")  # Mostrar primeros 3
    ...
    def callback_actualizar_metricas(
        ...
        try:
            ...
            if 'mejores_individuos' in historial and historial['mejores_individuos']:
                mejor_individuo = min(historial['mejores_individuos'], key=lambda x: x[1])[0]
            ...
            # Calcular fitness desde el historial (fuente única de verdad)
            if 'mejores_fitness' in historial and historial['mejores_fitness']:
                fitness_actual = historial['mejores_fitness'][-1]
                fitness = f"{fitness_actual:.3f}"
            elif estado_ag and 'mejor_fitness' in estado_ag and estado_ag['mejor_fitness'] != 0:
                fitness = f"{estado_ag['mejor_fitness']:.3f}"
            ...
            if 'diversidad' in historial and historial['diversidad']:
                diversidad_actual = historial['diversidad'][-1]
                diversidad = f"{diversidad_actual:.4f}"
            else:
                diversidad = "0.0000"
    ...
    # Callback para actualizar la barra de progreso del algoritmo
    @app.callback(
        [
            Output('barra-progreso', 'style'),
            Output('progreso-generacion', 'children'),
            Output('progreso-fitness', 'children'),
            Output('progreso-ultima', 'children')
        ],
        [
            Input('store-historial', 'data'),
            Input('store-ag-estado', 'data')
    ...
    def callback_actualizar_barra_progreso(historial, estado_ag):
        """
        Actualiza la barra de progreso y la información detallada del algoritmo.
        ...
        Returns:
            tuple: Estilo de la barra, texto de generación, fitness, y última mejora.
        """
        # Valores por defecto
        progress_style = {"width": "0%"}
        generacion_text = "0/100"
        fitness_text = "0.000"
        ultima_mejora_text = "Generación 0"

        try:
            if estado_ag:
                # Calcular porcentaje de progreso
                generacion_actual = estado_ag.get('generacion_actual', 0)
                num_generaciones = estado_ag.get('num_generaciones', 100)

                if num_generaciones > 0:
                    porcentaje = min(100, (generacion_actual / num_generaciones) * 100)
                    progress_style = {"width": f"{porcentaje}%"}

                # Texto de generación
                generacion_text = f"{generacion_actual}/{num_generaciones}"

                # Mejor fitness - usar historial como fuente principal
                if historial and 'mejores_fitness' in historial and historial['mejores_fitness']:
                    mejor_fitness = historial['mejores_fitness'][-1]
                    fitness_text = f"{mejor_fitness:.3f}"

                    # Última mejora (generación donde se encontró el mejor fitness absoluto)
                    mejores_fitness = historial['mejores_fitness']
                    generaciones = historial.get('generaciones', list(range(len(mejores_fitness))))

                    # Encontrar el mejor fitness absoluto (menor valor)
                    mejor_fitness_absoluto = min(mejores_fitness)
                    # Encontrar la generación donde se encontró este mejor fitness
                    indice_mejor = mejores_fitness.index(mejor_fitness_absoluto)
                    ultima_generacion_mejora = generaciones[indice_mejor] if indice_mejor < len(generaciones) else indice_mejor

                    ultima_mejora_text = f"Generación {ultima_generacion_mejora}"
                else:
                    # Fallback a estado_ag
                    mejor_fitness = estado_ag.get('mejor_fitness', 0)
                    fitness_text = f"{mejor_fitness:.3f}"

            return progress_style, generacion_text, fitness_text, ultima_mejora_text

        except Exception as e:
            print(f"Error al actualizar barra de progreso: {str(e)}")
            return progress_style, generacion_text, fitness_text, ultima_mejora_text
    ...
    def callback_actualizar_tabla(
        ...
        try:
            ...
            # Los mejores individuos ya están guardados como (individuo, fitness) en el algoritmo
            ...
            # Los mejores_individuos ya están ordenados por fitness en el algoritmo
            # Tomar los 10 mejores (ya están ordenados)
