...
def registrar_mapa_callbacks(app):
    ...
    def callback_actualizar_mapa(
        ...
        try:
            ...
            # Mostrar la ruta inicial (primer individuo) y la ruta optimizada (mejor individuo)
            ruta_inicial = None
            ruta_optimizada_coords = None

            if len(mejores_individuos) >= 1:
                # El primer individuo guardado es la ruta inicial
                ruta_inicial_individuo = mejores_individuos[0][0]
                ruta_inicial = convertir_ruta_a_coordenadas(ruta_inicial_individuo, df_puntos)

            # La ruta optimizada es siempre el mejor individuo encontrado
            ruta_optimizada_coords = ruta_optimizada
    ...
    def callback_visualizacion_ruta_completa(historial, datos):
        ...
        """
        print(f"DEBUG: callback_visualizacion_ruta_completa - datos: {bool(datos)}, historial: {bool(historial)}")

        # Verificar que existen datos
        if not datos or 'puntos' not in datos:
            print("DEBUG: No hay datos de puntos")
            from app.components.mapa import crear_mapa_sin_datos
        ...
        # Convertir datos a DataFrames
        df_puntos = pd.DataFrame(datos['puntos'])
        print(f"DEBUG: DataFrame creado con {len(df_puntos)} puntos")
        ...
        if not historial or 'mejores_individuos' not in historial or not historial['mejores_individuos']:
            print(f"DEBUG: No hay historial válido - historial: {historial}")
            from app.components.mapa import crear_mapa_sin_datos
        ...
        try:
            ...
            print(f"DEBUG: mejores_individuos: {len(mejores_individuos)} individuos")

            if not mejores_individuos:
                print("DEBUG: Lista de mejores_individuos está vacía")
                from app.components.mapa import crear_mapa_sin_datos
            ...
            print(f"DEBUG: Mejor ruta encontrada: {mejor_ruta}")
            ...
            print("DEBUG: Visualización completa creada exitosamente")
            ...
            print("DEBUG: Información detallada creada")
            ...
            from app.components.mapa import crear_mapa_sin_datos
            info_error = html.P(f"Error al generar la visualización completa: {str(e)}",
