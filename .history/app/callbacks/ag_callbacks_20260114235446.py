...
def registrar_callbacks(app):
    ...
    def callback_actualizar_progreso(
        ...
        Returns:
            tuple: (historial, intervalo_disabled, btn_iniciar_disabled, btn_pausar_disabled)
        """
        global ag_instancia, ag_ejecutando
        ...
        try:
            ...
            # Retornar estados sin cambios si no terminó
            return (
                historial,
                intervalo_disabled,
    ...
    # Callbacks para actualizar métricas en tiempo real
    @app.callback(
        Output('metrica-distancia', 'children'),
        Input('store-historial', 'data'),
        State('store-datos', 'data')
    )
    def actualizar_metrica_distancia(historial, datos):
        """Actualiza la métrica de distancia total."""
        if not historial or not datos or 'mejores_individuos' not in historial:
            return "--"

        try:
            # Calcular distancia de la mejor ruta
            mejores_rutas = historial.get('mejores_individuos', [])
            if mejores_rutas:
                mejor_ruta = mejores_rutas[-1]
                # Calcular distancia aproximada (esto es simplificado)
                # En una implementación completa, usaríamos las coordenadas reales
                distancia_aprox = len(mejor_ruta) * 8.5  # Distancia aproximada por punto
                return ".2f"
        except:
            pass
        return "--"
    ...
    def actualizar_metrica_tiempo(historial, datos):
        """Actualiza la métrica de tiempo total."""
        if not historial or not datos or 'mejores_individuos' not in historial:
            return "--"

        try:
            # Calcular tiempo aproximado basado en distancia
            mejores_rutas = historial.get('mejores_individuos', [])
            if mejores_rutas:
                mejor_ruta = mejores_rutas[-1]
                distancia_aprox = len(mejor_ruta) * 8.5
                velocidad_promedio = 30  # km/h
                tiempo_horas = distancia_aprox / velocidad_promedio
    ...
    def actualizar_metrica_generacion(estado_ag, total_generaciones):
        """Actualiza la métrica de generación actual."""
        if not estado_ag:
            return "0/0"

        generacion_actual = estado_ag.get('generacion_actual', 0)
        return f"{generacion_actual}/{total_generaciones}"

    @app.callback(
        Output('metrica-mejora', 'children'),
        Input('store-historial', 'data')
    )
    def actualizar_metrica_mejora(historial):
        """Actualiza la métrica de mejora respecto a la inicial."""
        if not historial or 'mejores_fitness' not in historial:
            return "--"

        mejores_fitness = historial['mejores_fitness']
        if len(mejores_fitness) < 2:
            return "--"

        fitness_inicial = mejores_fitness[0]
        fitness_final = mejores_fitness[-1]

        if fitness_inicial == 0:
            return "--"

        mejora = ((fitness_inicial - fitness_final) / fitness_inicial) * 100
        return ".1f"
    ...
    def actualizar_metrica_fitness(estado_ag):
        """Actualiza la métrica de mejor fitness."""
        if not estado_ag:
            return "--"

        fitness = estado_ag.get('mejor_fitness', 0)
        return ".4f"
    ...
    def actualizar_metrica_diversidad(historial):
        """Actualiza la métrica de diversidad poblacional."""
        if not historial or 'diversidad' not in historial:
            return "--"

        diversidades = historial['diversidad']
        if diversidades:
            diversidad_actual = diversidades[-1]
            return ".4f"
