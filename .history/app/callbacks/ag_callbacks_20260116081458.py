...
def registrar_callbacks(app):
    ...
    def callback_iniciar_algoritmo(
        ...
        try:
            ...
            # Guardar la población inicial como ruta de referencia para comparación
            if hasattr(ag_instancia, 'poblacion') and ag_instancia.poblacion:
                # Tomar el primer individuo de la población inicial como "ruta inicial"
                ruta_inicial = ag_instancia.poblacion[0].copy()
                fitness_inicial = ag_instancia.evaluar_individuo(ruta_inicial)

                # Agregar la ruta inicial a mejores_individuos para comparación
                ag_instancia.mejores_individuos.append((ruta_inicial, fitness_inicial))
    ...
    def callback_actualizar_progreso(
        ...
        try:
            ...
            # Crear nuevo historial completo cada vez para asegurar persistencia
            historial = {
                'mejores_fitness': [],
                'fitness_promedio': [],
                'peores_fitness': [],
            ...
            # Si hay historial existente, copiar datos previos
            if historial_existente:
                for key in historial.keys():
                    if key in historial_existente and isinstance(historial_existente[key], list):
                        historial[key].extend(historial_existente[key])

            # Obtener TODOS los datos históricos del algoritmo de una vez
            try:
                if hasattr(ag_instancia, 'historial') and ag_instancia.historial:
                    # Copiar TODOS los datos históricos del algoritmo
                    for entry in ag_instancia.historial:
                        historial['mejores_fitness'].append(entry.get('mejor_fitness', 0))
                        historial['fitness_promedio'].append(entry.get('promedio_fitness', 0))
                        historial['peores_fitness'].append(entry.get('peor_fitness', 0))
                        historial['generaciones'].append(entry.get('generacion', 0))
                        # Usar la diversidad real calculada por el algoritmo
                        diversidad_actual = entry.get('diversidad', 0.0)
                        if diversidad_actual == 0.0 and hasattr(ag_instancia, 'obtener_estadisticas'):
                            # Si no hay diversidad en el historial, calcularla
                            stats_temp = ag_instancia.obtener_estadisticas()
                            diversidad_actual = stats_temp.get('diversidad', 0.0)
                        historial['diversidad'].append(diversidad_actual)

                    # Copiar TODA la lista de mejores individuos del algoritmo
                    if hasattr(ag_instancia, 'mejores_individuos') and ag_instancia.mejores_individuos:
                        historial['mejores_individuos'] = ag_instancia.mejores_individuos.copy()

                    print(f"DEBUG: Historial COPIADO completo - Total generaciones: {len(historial['mejores_fitness'])}, Última gen: {ag_instancia.generacion_actual}")

                elif hasattr(ag_instancia, 'obtener_estadisticas'):
                    # Fallback: usar obtener_estadisticas si no hay historial directo
                    ...
                    if stats:
                        ...
                        print(f"DEBUG: Historial actualizado con stats - Gen: {ag_instancia.generacion_actual}, Fitness: {stats.get('mejor_fitness', 0):.4f}")

            except Exception as stats_error:
                print(f"Error al copiar historial completo: {str(stats_error)}")
                import traceback
                traceback.print_exc()
            ...
            # Actualizar estado_ag con valores iniciales si no hay mejoras aún
            if not estado_ag_actualizado.get('mejor_fitness') and historial.get('mejores_fitness'):
                # Si hay historial pero estado_ag no se actualizó, usar el primer fitness
                mejores_fitness = historial.get('mejores_fitness', [])
                if mejores_fitness:
                    estado_ag_actualizado['mejor_fitness'] = mejores_fitness[0]
