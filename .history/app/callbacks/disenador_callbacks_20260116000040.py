...
@callback(
    ...
    [Input("store-dataset-manual", "data"),
     ...
     Input("mapa-disenador", "id")],  # Trigger adicional para asegurar ejecución inicial
...
def actualizar_mapa_disenador(dataset_manual, dataset_csv, dataset_generado, tab_activa):
    ...
    logger.info(f"Datos - Manual: {dataset_manual is not None}, CSV: {dataset_csv is not None}, Generado: {dataset_generado is not None}")
    ...
    if tab_activa == "manual":
        ...
        logger.info(f"Usando dataset manual: {len(dataset_activo) if dataset_activo else 0} puntos")
    elif tab_activa == "importar":
        dataset_activo = dataset_csv
        logger.info(f"Usando dataset CSV: {len(dataset_activo) if dataset_activo else 0} puntos")
    elif tab_activa == "generar":
        dataset_activo = dataset_generado
        logger.info(f"Usando dataset generado: {len(dataset_activo) if dataset_activo else 0} puntos")
    ...
    try:
        ...
        if 0 not in df_puntos['id'].values:
            # Si no hay depósito, agregar uno
