...
def registrar_graficos_callbacks(app):
    ...
    @app.callback(
        ...
        [
            Input('store-historial', 'data'),
            Input('store-datos', 'data'),
            Input('store-poblacion-actual', 'data')
        ]
    )
    def callback_actualizar_tabla(
        historial: Optional[Dict],
        datos: Optional[Dict],
        poblacion_actual: Optional[List]
    ) -> List[Dict]:
        """
        Actualiza la tabla de resultados con la población actual.
        
        Args:
            historial: Datos del historial de ejecución.
            datos: Datos de puntos y distancias.
            poblacion_actual: Población actual del algoritmo genético.
            
        Returns:
            list: Lista de diccionarios con los datos de la tabla.
        """
        if not poblacion_actual or not datos or 'distancias' not in datos or 'puntos' not in datos:
            return []
        ...
        try:
            ...
            df_resultados = preparar_datos_tabla(
                individuos_fitness=poblacion_actual,
